...
@click.option(
    "--extract", help="extract NRT rainfall raster data", default=False, is_flag=True
)
@click.option(
    "--transform",
    help="calculate rainfall data in pre-defined administrative areas",
    default=False,
    is_flag=True,
)
@click.option(
    "--send",
    help="specify a customed latest date YYYY-mm-dd until which the data should be extracted, by default it is the date before today",
    default=False,
    is_flag=True,
)
...
def run_nrt_rainfall_pipeline(country, extract, transform, send, save, dateend):
    ...
    pipe.run_pipeline(
        extract=extract, transform=transform, send=send, save=save, dateend=dateend
    )
